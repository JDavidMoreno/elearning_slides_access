odoo.define('elearning_slides_access.slide_category_one2many', function (require){
"use strict";
    ...
    _renderBodyCell: function (record, node, index, options){
        ...
        if (isSection){
            if (node.attrs.widget === "handle" ||
              ($cell.length === 1 && $cell[0].firstChild && $cell[0].firstChild.id === 'slide_manage_access'))
            {
                return $cell;
            } else if (node.attrs.name === "name"){
                var nbrColumns = this._getNumberOfCols() - 1; // leave one column for the button
                if (this.handleField){
                    nbrColumns--;
                }
...
// Make it accessible and extensible for the rest of the world
return {
  SectionListRenderer: SectionListRenderer,
  SectionFieldOne2Many: SectionFieldOne2Many
}
});